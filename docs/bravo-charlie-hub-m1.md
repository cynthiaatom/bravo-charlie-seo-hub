...
7. Ongoing heartbeats are authenticated with HMAC-SHA256 and must use a timestamp inside a five-minute signature-freshness window.
...
The five-minute heartbeat timestamp check limits signature age; it is not a nonce store and therefore is not described as strict replay prevention. M1 also pins signed heartbeat payloads to the WordPress origin established during registration so a connected site cannot silently move its Hub record to another origin.

## Database support

Bravo Charlie tables are defined for both OpenSEO database backends: D1/SQLite and PostgreSQL. A focused schema-parity test guards table, column, key, unique-constraint, and foreign-key drift between the two definitions.

## Verification gate

PR #1 must pass formatting, type checks, tests, worker build, schema parity, and reviewed D1/PostgreSQL migrations before merge.

## Follow-up

After PR #1 is green, configure the Hub secret in the deployment environment and connect the first controlled WordPress test site.
